...
class ConversationListDao implements IConversationList, ITable {
  ...
  ConversationListDao(this.storage) {
    //创建表
    storage.db.execute(
      'create table if not exists $tableName (id integer primary key autoincrement, cid integer, title	text, icon	text, updateAt	integer, messageCount	integer, lastMessage	text, stickTime	integer);',
    );
  ...
  Future<List<ConversationModel>> getConversationList({
    ...
    var results = await storage.db.rawQuery(
      'select * from $tableName where cast(stickTime as integer) <=0 order by updateAt desc limit $pageSize offset $offset',
    );

    // 将查询结果转成Dart Model以方便使用
    var list = results.map((item) => ConversationModel.fromJson(item)).toList();
    debugPrint('ConversationList count:${list.length}');
  ...
  Future<List<ConversationModel>> getStickConversationList() async {
    ...
    var results = await storage.db.rawQuery(
      'select * from $tableName where cast(stickTime as integer) > 0 order by stickTime desc;',
    );
