...
class FavoriteDao implements IFavorite, ITable {
  ...
  FavoriteDao(this.storage) {
    // 创建表
    storage.db.execute(
      'create table if not exists $tableName (id integer primary key autoincrement, ownerName text, content text, createdAt integer);',
    );
  }

  @override
  Future<int?> addFavorite(FavoriteModel model) async {
    var result = await storage.db.insert(
      tableName,
      model.toJson(),
